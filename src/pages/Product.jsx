...
const Product = () => {
  ...
  const handleClick = () => {
    if(!user){NotificationManager.error('Please login to proceed!',"",2000)}
        ...
        const {color,size,...rest}=product;
        const newproduct=rest;
        newproduct.color=colorr;
        newproduct.size=sizee;
        newproduct.quantity=quantity;
        addproduct(dispatch, {...newproduct,userid:user._id});}
          ...
          <FilterContainer>
            <Filter>
              ...
              {product.color?product.color.map((c) => (
                <FilterColor color={c} key={c} onClick={() => setColorr(c)} />
              )): null}
              
            </Filter>
            <Filter>
              <FilterTitle>Size</FilterTitle>
              <FilterSize onChange={(e) => setSizee(e.target.value)}>
